...
public interface TagSectionDao
{
    ...
    @Query("SELECT * FROM " + Constants.TABLE_TAG_SECTION +" ORDER BY tagSectionName ASC")
    LiveData<List<TagSection>> getAll();

    @Query("SELECT COUNT(*) FROM " + Constants.TABLE_TAG_SECTION +" WHERE tagSectionId LIKE :tagSectionId LIMIT 1")
